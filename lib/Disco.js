var crypto = require('crypto')
  ...
  , _ = require('lodash')
  , ip = require('ip')
  , FailureDetector = require('failure-detector').FailureDetector
...
Disco.prototype._defaultEventHandler = function _defaultEventHandler(evtName, msg) {
  ...
  if (!this.fds[node.id]) {
    this.fds[node.id] = new FailureDetector()
  }

  this.fds[node.id].report()
  
  node.phi = this.fds[node.id].phi.bind(this.fds[node.id])
